...
class CIMHeader {
    ...
    func readHeader(data: Data) -> Bool {
        ...
        length = len
        version = (UInt16(buffer[4]) << 8) + UInt16(buffer[5])
    ...
    class func uintToBytes(num: UInt16) -> [UInt8] {
        return [UInt8(truncatingIfNeeded: num), UInt8(truncatingIfNeeded: num >> 8)]
    }

    /// 转成 4字节的bytes
    class func uintToFourBytes(num: UInt32) -> [UInt8] {
        return [UInt8(truncatingIfNeeded: num), UInt8(truncatingIfNeeded: num >> 8), UInt8(truncatingIfNeeded: num >> 16), UInt8(truncatingIfNeeded: num >> 24)]
    }
