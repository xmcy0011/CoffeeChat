/// IM 用户配置（登录前设置）
class IMUserConfig {
  Function onForceOffline; // 被其他终端踢下线
  Function onUserSigExpired; // 用户签名过期了，需要刷新 userSig 重新登录 IM SDK

  Function onConnected; // 连接上IM服务器
  Function onDisconnected; // 与IM服务器连接断开
  Function onWifiNeedAuth; // WIFI需要密码认证

  Function onRefresh; // 数据刷新通知回调（如未读计数，会话列表等）
  Function onRefreshConversation; // 部分会话刷新（包括多终端已读上报同步）
  ...
  /// 设置群组事件回调对象
  /// [onGroupTipsEvent] void onGroupTipsEvent(TIMGroupTipsElem elem)
//void setGroupEventListener(Function onGroupTipsEvent) {}

  /// 设置会话刷新回调对象
  /// [onRefresh] void onRefresh() 数据刷新通知回调（如未读计数，会话列表等）
  /// [onRefreshConversation] void onRefreshConversation(List<TIMConversation> conversations) 部分会话刷新（包括多终端已读上报同步）
