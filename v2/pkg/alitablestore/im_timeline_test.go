...
func setup(t *testing.T) *IMTimeLine {
	//set builder config
	...
	im, err := NewIMTimeLine(storeBuilder, syncBuilder, timeline.DefaultStreamAdapter)
	if err != nil {
		t.Fatal("init im timeline failed: ", err)
	}
	return im
}

func TestNewIMTimeLine(t *testing.T) {
	im := setup(t)
	...
	if err != nil {
		t.Fatal("user_A send message to group failed: ", err)
	}
	if len(faileds) != 0 {
		t.Fatal("user_A send message to some user failed, failed user list:", faileds)
	}
	...
	if err != nil {
		t.Fatal("user_A send message to user_B: ", err)
	}
	...
	if err != nil {
		t.Fatal("user_B get sync messages failed: ", err)
	}
	...
	if err != nil {
		t.Fatal("user_B get history messages failed: ", err)
	}
	for _, entry := range entries {
