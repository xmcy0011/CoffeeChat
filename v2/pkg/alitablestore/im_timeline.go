...
var kSingleChatStoreNameJoiner = ":"

type IMTimeLine struct {
...
// NewIMTimeLine new timeline wrapper
func NewIMTimeLine(storeOption, syncOption timeline.StoreOption, adapter timeline.MessageAdapter) (*IMTimeLine, error) {
	//c := tablestore.NewTimeseriesClient(syncOption.Endpoint, syncOption.Instance, syncOption.AkId, syncOption.AkSecret)
	//timeline.NewMessageStore(c, storeOption)
...
func (im *IMTimeLine) GetSyncMessage(member string, lastRead int64) ([]*timeline.Entry, error) {
...
func (im *IMTimeLine) GetHistoryMessage(storeName string, numOfHistory int) ([]*timeline.Entry, error) {
...
func (im *IMTimeLine) Send(from, to string, message timeline.Message) error {
...
func (im *IMTimeLine) SendGroup(groupName string, groupMembers []string, message timeline.Message) ([]string, error) {
...
func singChatStoreName(userA, userB string) string {
	if userA > userB {
		return fmt.Sprintf("%s%s%s", userB, kSingleChatStoreNameJoiner, userA)
	}
	return fmt.Sprintf("%s%s%s", userA, kSingleChatStoreNameJoiner, userB)
}
