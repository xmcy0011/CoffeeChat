...
> 2020.06.09 更新-------------  
> gate和logic之间优化使用MQ通信，削峰、解耦、吞吐量、实时/离线统计计算。

以下是基于MQ的架构（为什么是RocketMQ，见[MQ在IM中的实践和选型](https://github.com/xmcy0011/CoffeeChat/blob/master/docs/06_MQ%e5%9c%a8IM%e4%b8%ad%e7%9a%84%e5%ae%9e%e8%b7%b5.md)）：  
![模块架构](https://raw.githubusercontent.com/xmcy0011/CoffeeChat/master/images/structure-v2.png)

引入MQ后，Gate和Logic之间如何确保消息一致性，发送的消息确保成功了（原来是通过gRPC同步返回结果，现在变成异步了，需要有一个ACK机制）？  
答案：MQ事物消息。
...
![单聊](https://raw.githubusercontent.com/xmcy0011/CoffeeChat/master/images/seq-c2c-robot.png)
...
1. App 端登录成功，点击某个聊天后，向 IM 系统发起拉历史离线消息请求。传递 3 个主要参数，uid 表明用户；end_msg_id 表明当前收到的最小消息 id（如果没收到过消息，或拿不到最小消息 id 则为 0）即可；limit 表示每次拉取条数（这个值也可以由服务器端控制）。
...
### 1.3.11 音视频实时通话
![音视频](https://raw.githubusercontent.com/xmcy0011/CoffeeChat/master/images/architecture_voice_video_call.png)
